...
export default function FileCase() {
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4"><h1 className="page-title h3 mb-0">{t('File New Case')}</h1></div>
      <div className="card shadow-sm" style={{ maxWidth: 640 }}>
        <div className="card-body">
          {err && <div className="alert alert-danger py-2">{err}</div>}
          <form onSubmit={submit}>
            <div className="mb-3">
              <label className="form-label fw-semibold small">{t('Case Title')}</label>
            ...
            <div className="mb-3">
              <label className="form-label fw-semibold small">{t('Litigant ID (user email or id)')}</label>
              <input className="form-control" value={form.litigantId} onChange={e => setForm({ ...form, litigantId: e.target.value })} required />
            </div>
            <div className="mb-3">
              <label className="form-label fw-semibold small">{t('Lawyer ID (optional)')}</label>
              <input className="form-control" value={form.lawyerId} onChange={e => setForm({ ...form, lawyerId: e.target.value })} />
            </div>
            <button className="btn btn-dark" disabled={loading}>{loading ? t('Filing...') : t('File Case')}</button>
          </form>
        </div>
      </div>
    </div>
