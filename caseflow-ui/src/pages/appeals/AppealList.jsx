  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h1 className="page-title h3 mb-0">{t('Appeals')}</h1>
        {canFile && <Link to="/appeals/file" className="btn btn-dark">+ {t('File Appeal')}</Link>}
      </div>
      <div className="card shadow-sm">
        <div className="card-body">
          ...
          {loading ? <div className="text-center text-muted py-4">{t('Loading...')}</div> : list.length === 0 ? <div className="text-center text-muted py-4">{t('No appeals found')}</div> : (
            <div className="table-responsive">
              <table className="table table-hover align-middle">
                <thead className="table-light"><tr><th>{t('ID')}</th><th>{t('Case')}</th><th>{t('Litigant')}</th><th>{t('Date')}</th><th>Reason</th><th>{t('STATUS')}</th><th></th></tr></thead>
                <tbody>
                  {list.map(a => (
                    <tr key={a.appealId}>
                      <td>#{a.appealId}</td>
                      ...
                      <td><Link to={`/appeals/${a.appealId}`} className="btn btn-outline-secondary btn-sm">{t('Open')}</Link></td>
                    </tr>
                  ))}
                </tbody>
              </table>
