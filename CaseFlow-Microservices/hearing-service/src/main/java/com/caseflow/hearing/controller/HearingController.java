...
public class HearingController {
    ...
    public ResponseEntity<ScheduleResponse> addSlot(
        ...
        denyAdminForScheduling(userRole);
        return ResponseEntity.ok(hearingService.addScheduleSlot(request));
    }
    @GetMapping("/schedule/judge/{judgeId}/available") public ResponseEntity<List<ScheduleResponse>> getAvailableSlots(@PathVariable String judgeId) { return ResponseEntity.ok(hearingService.getAvailableSlotsByJudge(judgeId)); }
    @GetMapping("/schedule/judge/{judgeId}/date/{date}") public ResponseEntity<List<ScheduleResponse>> getSlotsByDate(@PathVariable String judgeId, @PathVariable @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) { return ResponseEntity.ok(hearingService.getSlotsByJudgeAndDate(judgeId, date)); }
    @GetMapping("/schedule/judge/{judgeId}/all") public ResponseEntity<List<ScheduleResponse>> getAllSlots(@PathVariable String judgeId) { return ResponseEntity.ok(hearingService.getAllSlotsByJudge(judgeId)); }
    ...
    private void denyAdminForScheduling(String userRole) {
